    ...
    <Grid container spacing={3} justify="center">
        ...
        <Paper variant="outlined" elevation={10} className={classes.paper}>
          ...
          >
            <Typography variant="h6" align="center">
              Easy problems ({easyProbs.length})
            </Typography>
            {easyProbs.length ? (
              easyProblems.map(
                (item, index) =>
                  index < 5 && (
                    <Chip
                      ...
                      label={`${item.problem.index}) ${item.problem.name} (${item.problem.rating})`}
                      component="a"
                      href={`https://codeforces.com/contest/${item.problem.contestId}/problem/${item.problem.index}`}
                      target="_blank"
                      clickable
                    />
                  )
              ...
              <Typography className={classes.typography}>
                You don't have any questions left in this bracket!!!!
              </Typography>
            )}
            {easyProbs.length > 5 && (
              <Button
      ...
      <Grid item xs={8} sm={3}>
        <Paper variant="outlined" elevation={10} className={classes.paper}>
          ...
          >
            <Typography variant="h6" align="center">
              Medium problems ({mediumProbs.length})
            </Typography>
            {mediumProbs.length ? (
              mediumProblems.map(
                (item, index) =>
            ...
            {mediumProbs.length > 5 && (
              <Button
                color="secondary"
                size="small"
                variant="contained"
                size="small"
                onClick={() => setMedium(true)}
                style={{ width: "50%", alignSelf: "center" }}
              >
                Randomize
              </Button>
      ...
      <Grid item xs={8} sm={3}>
        <Paper variant="outlined" elevation={10} className={classes.paper}>
          ...
          >
            <Typography variant="h6" align="center">
              Hard problems ({hardProbs.length})
            </Typography>
            {hardProbs.length ? (
              hardProblems.map(
                (item, index) =>
      ...
      <Grid item xs={8} sm={3}>
        <Paper variant="outlined" elevation={10} className={classes.paper}>
          ...
          >
            <Typography variant="h6" align="center">
              Very hard problems ({veryHardProbs.length})
            </Typography>
            {veryHardProbs.length ? (
              veryHardProblems.map(
                (item, index) =>
                  index < 5 && (
                    <Chip
                      className={classes.chip}
            ...
            {veryHardProbs.length > 5 && (
              <Button
                color="secondary"
                size="small"
                variant="contained"
                size="small"
                onClick={() => setVeryHard(true)}
                style={{ width: "50%", alignSelf: "center" }}
              >
                Randomize
              </Button>
