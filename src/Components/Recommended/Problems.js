...
export const getRecommendedProblems = async (userHandle) => {
  ...
  for (let i = 0; i < totalProbs; i++) {
    ...
    for (let j = 0; j < ACprobs.length; j++) {
      if (
        ACprobs[j].contestId === probSet.data.result.problems[i].contestId &&
        ACprobs[j].probIdx === probSet.data.result.problems[i].index
      ) {
        ok = true;
        break;
      }
  ...
  for (let i = 8; i < unSolvedProbs.length; i++) {
    if (avgRating < 1000) {
      if (unSolvedProbs[i].rating <= 1000) {
        easyProbs.push(unSolvedProbs[i]);
      } else if (
        unSolvedProbs[i].rating > 1000 &&
        unSolvedProbs[i].rating <= 1300
      ) {
        mediumProbs.push(unSolvedProbs[i]);
      } else if (
        unSolvedProbs[i].rating > 1300 &&
        unSolvedProbs[i].rating <= 1800
      ) {
    ...
    if (avgRating > 1000 && avgRating < 2000) {
      if (unSolvedProbs[i].rating < avgRating - 200) {
        ...
        unSolvedProbs[i].rating >= avgRating - 200 &&
        unSolvedProbs[i].rating <= avgRating + 200
      ) {
        mediumProbs.push(unSolvedProbs[i]);
      } else if (
        unSolvedProbs[i].rating > avgRating + 200 &&
        unSolvedProbs[i].rating <= avgRating + 600
      ) {
        hardProbs.push(unSolvedProbs[i]);
      } else {
        veryHardProbs.push(unSolvedProbs[i]);
      ...
      if (unSolvedProbs[i].rating <= 1600) {
        ...
        unSolvedProbs[i].rating > 1600 &&
        unSolvedProbs[i].rating <= 2200
      ) {
        mediumProbs.push(unSolvedProbs[i]);
      } else if (
        unSolvedProbs[i].rating > 2200 &&
        unSolvedProbs[i].rating <= 2800
      ) {
        hardProbs.push(unSolvedProbs[i]);
      } else {
        veryHardProbs.push(unSolvedProbs[i]);
      ...
      if (unSolvedProbs[i].rating <= 1800) {
        ...
        unSolvedProbs[i].rating > 1800 &&
        unSolvedProbs[i].rating <= 2400
      ) {
        mediumProbs.push(unSolvedProbs[i]);
      } else if (
        unSolvedProbs[i].rating > 2400 &&
        unSolvedProbs[i].rating <= 2900
      ) {
        hardProbs.push(unSolvedProbs[i]);
      } else {
        veryHardProbs.push(unSolvedProbs[i]);
